...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        - reg: Regularization strength.  正则化强度
        ...
        z1 = np.dot(X, W1) + b1
        a1 = np.maximum(0, z1)  # 保证取到的是正数，激活函数为Relu
        ...
        loss = -np.sum(np.log(temp[range(N), y].reshape(-1, 1) / np.sum(temp, axis=1).reshape(-1, 1)))
        loss /= N
        ...
        dz2 = temp / np.sum(temp, axis=1).reshape(-1, 1)
        dz2[range(N), y] -= 1
    ...
    def predict(self, X):
        ...
        y_pred = np.argmax(scores, axis=1)  # 找得分最高的
